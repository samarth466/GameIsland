...
class Rook(Piece):
    ...
    def move(self, squares, win, board):
        if not isinstance(squares, list):
            raise TypeError('The squares attribute must be a list(), not ' +
                            str(type(squares))[8:-1].capitalize()+'().')
        ...
        while (self.x in limiting_pos[0] and self.y in limiting_pos[1]):
            ...
            if keys[pygame.K_KP2] or keys[pygame.K_2]:
                _, self.y = self._move(
                    self.x, self.y, 0, self.square_height, squares)
                self.piece_y = self.y
                self.has_moved = True
            if keys[pygame.K_KP4] or keys[pygame.K_4]:
                self.x, _ = self._move(
                    self.x, self.y, -self.square_width, 0, squares)
                self.piece_x = self.x
                self.has_moved = True
            ...
            while direction < max_direction:
                if direction == 0:
                    self.attacked_pieces = self._update_attacked_pieces(
                        direction+direction_offset, self.x, self.y, 0, -square_height, self.square_width, self.square_height, squares, self.color, lower_y_bound=0, upper_y_bound=self.win_height)
                if direction == 1:
                    self.attacked_pieces = self._update_attacked_pieces(
                        direction+direction_offset, self.x, self.y, -self.square_width, 0, square_width, square_height, squares, self.color, lower_x_bound=0, upper_x_bound=self.win_width)
                if direction == 2:
                    self.attacked_pieces = self._update_attacked_pieces(direction+direction_offset, self.x, self.y, 0, self.square_height, self.square_width, self.square_height, squares, self.color, lower_y_bound=0, upper_y_bound=self.win_height)
                if direction == 3:
                    self.attacked_pieces = self._update_attacked_pieces(direction+direction_offset, self.x, self.y,     self.square_width, 0, self.square_width, self.square_height, squares, self.color, lower_x_bound=0, upper_x_bound=self.win_width)
